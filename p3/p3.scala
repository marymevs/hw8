...
class Definition(list : List[Production]) extends GrammarElement { 
  override def expand(grammar : Grammar) : String = {
    println(list.length);
//    val number = Random.nextInt(list.length - 5) + 5;
    return list(0).expand(grammar);
  }
...
object RandomSentenceGenerator {
  ...
  def main(args: Array[String]) = {
    ...
    val grammar = this.readGrammar( new Scanner(is));
    
    //first random sent here
